...
def check_T_field():
    """T_field: Field Content from Anomaly Uniqueness + Capacity Cost.
    
    UPGRADED v3.6: [C] → [P_structural].
    
    Previously declared as regime input. Now derived:
    
    DERIVATION CHAIN (all [P] except step 4):
      1. T_gauge [P]: gauge group = SU(3)×SU(2)×U(1)
      2. T4 [P]: anomaly cancellation required (Adler-Bell-Jackiw)
      3. T_channels [P]: exactly 4 EW channels, split (3 mixer, 1 bookkeeper)
         → 3 colored rep types + 1 leptonic rep type
      4. A5 (minimality) + capacity cost ordering → fundamental reps only
         [STRUCTURAL: Dynkin index ordering from A5, not pure logic]
      5. Anomaly Diophantine system (4 equations, 5 unknowns) has
         UNIQUE solution (up to normalization):
           Y_Q = 1/6, Y_u = 2/3, Y_d = -1/3, Y_L = -1/2, Y_e = -1
         This IS the SM field content {Q_L, L_L, u_R, d_R, e_R}.
         [P: exact rational arithmetic, verified algebraically]
    
    Remaining gap: Step 4 (why fundamental reps). A5 minimality +
    T0.2a' capacity bounds give strong motivation but not pure logical force.
    """
    from fractions import Fraction
    
    # === Anomaly uniqueness computation ===
    # Given (3 colored, 1 leptonic) channel structure from T_channels:
    #   Q_L = (3,2,Y_Q), u_R = (3,1,Y_u), d_R = (3,1,Y_d)
    #   L_L = (1,2,Y_L), e_R = (1,1,Y_e)
    
    # Anomaly equations:
    # (1) SU(3)²×U(1):  2*Y_Q - Y_u - Y_d = 0
    # (2) SU(2)²×U(1):  3*Y_Q + Y_L = 0  
    # (3) Gravitational: 6*Y_Q - 3*Y_u - 3*Y_d + 2*Y_L - Y_e = 0
    # (4) U(1)³: cubic constraint
    
    # Solve: set Y_Q = 1/6 (normalization)
    Y_Q = Fraction(1, 6)
    Y_L = -3 * Y_Q              # from (2): Y_L = -1/2
    Y_e = -6 * Y_Q              # from (3): Y_e = -1
    # From (1): Y_u + Y_d = 2*Y_Q = 1/3
    # From (4): cubic constraint → Y_u*Y_d = -2/9
    # → Y_u, Y_d are roots of t² - (1/3)t - 2/9 = 0
    # → t = (1/3 ± 1)/2 → Y_u = 2/3, Y_d = -1/3
    Y_u = Fraction(2, 3)
    Y_d = Fraction(-1, 3)
    
    # Verify all 4 anomaly equations
    eq1 = 2*Y_Q - Y_u - Y_d                                          # SU(3)²×U(1)
    eq2 = 3*Y_Q + Y_L                                                 # SU(2)²×U(1)
    eq3 = 6*Y_Q - 3*Y_u - 3*Y_d + 2*Y_L - Y_e                       # gravitational
    eq4 = 6*Y_Q**3 - 3*Y_u**3 - 3*Y_d**3 + 2*Y_L**3 - Y_e**3       # U(1)³ cubic
    ...
    regime = {
        ...
        'derivation_status': 'unique solution of Diophantine system',
    }
    
    return _result(
        name='T_field: Field Content (Anomaly Uniqueness)',
        tier=2,
        epistemic='P_structural',
        summary=(
            'Field content {Q_L, L_L, u_R, d_R, e_R} DERIVED (not assumed). '
            'T_gauge [P] → SU(3)×SU(2)×U(1). T_channels [P] → 4 channels, (3,1) split. '
            'A5 minimality → fundamental reps (structural). '
            'Anomaly Diophantine system has UNIQUE solution: '
            f'Y_Q={Y_Q}, Y_u={Y_u}, Y_d={Y_d}, Y_L={Y_L}, Y_e={Y_e}. '
            f'All 4 anomaly equations verified = 0. '
            'Remaining gap: A5 → fundamental reps (structural, not logical). '
            'UPGRADED v3.6: [C] → [P_structural].'
        ),
        key_result='Field content uniquely determined by anomaly cancellation (Y_Q=1/6, Y_u=2/3, Y_d=-1/3)',
        dependencies=['T_gauge', 'T4', 'T_channels', 'A5'],
...
def check_T6B():
    ...
    RED TEAM NOTE (v3.6): One-loop SM running from 3/8 at M_U gives
    ~0.285 at M_Z, NOT 0.231. This is the well-known gauge coupling
    unification problem of the non-SUSY Standard Model. The framework's
    PRIMARY sin²θ_W derivation is the fixed-point (T24, which gives
    3/13 = 0.2308 with 0.19% error) and does NOT depend on T6 or T6B.
    T6B is a supplementary consistency check that reveals where
    higher-order corrections / threshold effects would enter.
    """
    sin2_MU = 3.0 / 8.0  # = 0.375
    sin2_MZ_target = 0.2312  # experimental
    sin2_MZ_oneloop = 0.285  # actual one-loop result
    sin2_fp = 3.0 / 13.0    # fixed-point (T24)
    ...
    return _result(
        ...
        summary=(
            ...
            f'One-loop SM running gives ~{sin2_MZ_oneloop:.3f} (not {sin2_MZ_target}). '
