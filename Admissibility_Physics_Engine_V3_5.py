...
def _dark_sector_theorems() -> Dict[str, Any]:
    ...
    # T12E: BARYON FRACTION — quantitative prediction
    # f_b = C_visible / C_total = 1 / (1 + α_eff)
    # α_eff = C_dark / C_visible
    # C_visible = C_gauge = 12 (from T_gauge)
    # C_dark is NOT independently derived → α_eff is the gap
    alpha_eff = 4.0   # Structural estimate (not derived)
    f_b = 1.0 / (1.0 + alpha_eff)  # = 0.200
    f_b_observed = 0.157  # Planck 2018

    # What α_eff WOULD match observation:
    alpha_obs = (1.0 / f_b_observed) - 1.0  # ≈ 5.37
    ...
    return {
        ...
        'T12E': {
            'name': 'T12E: Baryon Fraction f_b (Quantitative)',
            'tier': 4,
            'passed': True,
            'epistemic': 'P_structural',
            'summary': (
                f'f_b = 1/(1+α_eff) = {f_b:.3f} where α_eff = C_dark/C_visible. '
                f'C_visible = {C_gauge} (from T_gauge, proven). '
                f'α_eff = {alpha_eff} is a STRUCTURAL ESTIMATE (not derived). '
                f'Predicted: f_b = {f_b:.3f}. Observed: {f_b_observed}. '
                f'Error: {abs(f_b - f_b_observed)/f_b_observed*100:.1f}%. '
                f'Matching observation requires α_eff ≈ {alpha_obs:.2f}. '
                'GAP: C_total (hence α_eff) not independently derived. '
                'T11 ↔ T12 ledger audit: MECE confirmed — '
                'Λ + DM account for full capacity residual.'
            ),
            'key_result': f'f_b = {f_b:.3f} (obs: {f_b_observed}); α_eff gap honest',
...
GAP_REGISTRY = {
    ...
    'T12E': {'anchor': 'f_b = 1/(1+α_eff)', 'gap': 'OPEN PHYSICS', 'to_close': 'Derive C_total (same UV gap as T10/T11)'},
    # TIER 5
...
def _classify_gap(tid: str) -> str:
    ...
    imports = {
        'T6B',  # Scale identification gap (β-formula derived, capacity↔momentum not)
    }
    open_physics = {'T4G', 'T4G_Q31', 'T10', 'T11', 'T12E'}  # T12E: α_eff needs C_total
...
def export_json(master: Dict[str, Any]) -> str:
    ...
    audit_checks = [
        ...
        {'id': 'A21', 'check': 'T12E reclassified: regime_dependent → open_physics (α_eff = C_total gap)', 'status': 'FIXED', 'severity': 'medium'},
    ...
    ps_reasons = {}
    open_phys = {'T4G', 'T4G_Q31', 'T10', 'T11', 'T12E'}  # T12E: α_eff needs C_total (same UV gap as T10/T11)
